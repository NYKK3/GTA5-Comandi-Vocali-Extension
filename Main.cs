...
namespace GTA5ComandiVocaliExtension
{
    public class Main : Plugin
    {
        ...
        public static byte[] ProcessYourResponse(string data)
        {
            ...
            if (data == "fermato_un_veicolo_backup2") { SafeCall(() => UltimateBackup.API.Functions.callTrafficStopBackup()); }
            if (data == "fermato_un_veicolo_backup") { SafeCall(() => Backup.FelonyStopBackup()); } // potrebbe essere simile a questa sopra
            if (data == "codice2") { SafeCall(() => Backup.Code2()); }
            if (data == "codice3") { SafeCall(() => Backup.Code3()); }
            if (data == "codice4") { SafeCall(() => Status.Code4()); }
            if (data == "swat") { SafeCall(() => Backup.Swat()); }
            if (data == "noose") { SafeCall(() => Backup.NOOSE()); }
            if (data == "controllo_radio") { SafeCall(() => Status.RadioCheck()); }
            ...
            if (data == "invia_controllo_animali") { SafeCall(() => Backup.Animal()); }
            if (data == "si") { SafeCall(() => Interactions.Yes()); }
            if (data == "no") { SafeCall(() => Interactions.No()); }
            if (data == "multa") { SafeCall(() => Interactions.TicketNumber()); }
            if (data == "taser") { SafeCall(() => Interactions.Taser()); }
            if (data == "contatto_visivo") { SafeCall(() => Advisory.Visual()); }
            if (data == "persona_ferita") { SafeCall(() => Advisory.PersonInjured()); }
            if (data == "persona_deceduta") { SafeCall(() => Advisory.PersonDeceased()); }
            if (data == "lotta") { SafeCall(() => Advisory.Fight()); }
            if (data == "stop_traffico") { SafeCall(() => Traffic.Stop()); }
            if (data == "rallenta_traffico") { SafeCall(() => Traffic.Slow()); }
            if (data == "riprendi_traffico") { SafeCall(() => Traffic.Clear()); }
            if (data == "controllo_traffico") { SafeCall(() => Traffic.Control()); }
            if (data == "ufficiale_donna") { SafeCall(() => Backup.Female()); }
            if (data == "k9") { SafeCall(() => Backup.K9()); }
